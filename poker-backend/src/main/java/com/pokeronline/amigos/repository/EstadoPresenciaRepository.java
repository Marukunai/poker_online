...
public interface EstadoPresenciaRepository extends JpaRepository<EstadoPresencia, Long> {
    ...
    @Query("update EstadoPresencia e set e.ultimaActividad = :ts where e.userId = :userId")
    int touch(@Param("userId") Long userId, @Param("ts") LocalDateTime timestamp);
    ...
    int updateSnapshot(@Param("userId") Long userId,
                       @Param("estado") com.pokeronline.amigos.model.EstadoConexion estado,
